...
legendSvg.append("text")
  ...
  .attr("y", 20)
  .attr("id", "legendTitle")
  .text("Brecha entre el valor de los cupones y el costo de una comida")
...
d3.select("#bars").append("div")
  .attr("id", "tt-text")
  .attr("class","toRemove")
  .html("El costo promedio de una comida es <span id = \"tt-dollars\">" + DOLLARS(US_MEAL) + "</span>, <span id = \"tt-percent\">" + PERCENT(1 - SNAP_COST/ US_MEAL) + " m&aacute;s</span> que el valor de los cupones.")
...
function restoreNational(){
  ...
  d3.select("#tt-percent").html(PERCENT(US_RATIO) + " m&aacute;s")
  d3.select("#countyLabel").text("Promedio Nacional")
...
zoomOut.append("text")
  ...
  .attr("id", "zoomOut")
  .attr("text-anchor","middle")
  .text("Volver a Nacional")

function mouseover(d){
    var ratio = (+d.properties.cost - SNAP_COST)/SNAP_COST

    d3.select("#tt-percent").html(function(){
      var moreLess = (ratio < 0) ? " menos" : " m&aacute;s"
